...
export class ActiviteUserComponent implements OnInit {
  ...
  constructor(private activiteService: CrudActiviteService,
              ...
              private feedbackService: CrudFeedbackService,
  ...
  getFeedbacks(activite: any): any {
    let feedbacks: any = [];
    this.feedbackService.getAllFeedbacksByActiviteId(activite).subscribe(data => feedbacks = data);
    return feedbacks.map((feedback: { description: string; }) => feedback.description);
  }
